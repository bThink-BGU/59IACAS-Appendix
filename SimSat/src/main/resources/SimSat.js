...
bp.registerBThread("EPS - Turn ON/OFF logic", function () {
    ...
    if (ePSTelem.vBatt >= good_Min) {
        bp.sync({waitFor: [EPSTelem], request: bp.Event("SetEPSModeGood")});
    } else if (ePSTelem.vBatt >= low_Min) {
        bp.sync({waitFor: [EPSTelem], request: bp.Event("SetEPSModeLow")});
    } else {
        bp.sync({waitFor: [EPSTelem], request: bp.Event("SetEPSModeCritical")});
    }
        ...
        if (aDCSEvent.equals(bp.Event("ActivePass"))) {
            ...
            if (aDCSEvent.currentADCSMode == "Detumbling") {
                if (aDCSEvent.angularRate == "Low" && activePass) {
                    bp.sync({waitFor: [ADCSTelem], request: bp.Event("SetADCSModePayloadPointing")});
                } else if (aDCSEvent.angularRate == "Low") {
                    bp.sync({waitFor: [ADCSTelem], request: bp.Event("SetADCSModeSunPointing")});
                }
            }
            ...
            if (aDCSEvent.currentADCSMode == "SunPointing") {
                if (aDCSEvent.angularRate == "Low" && activePass) {
                    ...
                    bp.sync({waitFor: [ADCSTelem], request: bp.Event("SetADCSModeDetumbling")});
                }
            }
...
bp.registerBThread("EPS & ADCS Integrator", function () {
    while (true) {
        ...
        while (ePSTelem2.currentEPSMode == "Low" || ePSTelem2.currentEPSMode == "Critical") {
            if (activePass = true)
                bp.sync({waitFor: [ADCSTelem], request: bp.Event("PassDone"),
